...
On the `Stop` event only, and checked before both the presence check and the loop-demotion predicate, the hook exits 0 immediately (no warn text, logs `skipped=headless`) when `CODERAILS_HEADLESS_RUN=1` is present in the process env — a third condition alongside "outside a loop → block" and "inside an active incomplete loop → warn." `SubagentStop` ignores the flag entirely and stays block-enforced regardless — a headless run's own spawned subagents get no pass. The flag is set in exactly one place in the codebase: the dashboard's `POST /api/run` route's `spawn(...)` call, for a headless `claude -p` run with no interactive turn left to satisfy a repair-turn block — without the exemption the gate would displace the run's actual answer with gate-repair text. `AGENTS.md` explicitly flags any PR introducing a second set-site as a security finding, not a legitimate extension. See [[enforcement-model]]'s "headless-run exemption" section and [[pr_163-168_dashboard-rethink]] for the source record.
...
6. **Headless-run exemption** (2026-07-14, PR #167): added a `CODERAILS_HEADLESS_RUN=1` Stop-only exit-0 exemption, checked before both the presence and bullet-tagging checks, for the dashboard's headless `claude -p` run route. See [[pr_163-168_dashboard-rethink]].
