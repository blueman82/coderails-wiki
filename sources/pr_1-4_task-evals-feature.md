...
- **`install.sh`** does not yet reference `scripts/lib/eval-artifact.sh` or `scripts/post_evals.sh` by name in its own inventory/audit surface the way it does other core scripts — flagged, not fixed.
