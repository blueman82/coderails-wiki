...
>
> The shipped example config is rotted further still: `examples/dashboard-config.json` pins this routine's `artifactPath` to `-Users-harrison-Documents-Github-coderails-.git`, a `Documents/` checkout that does not exist — the repo lives at `~/Github/coderails`. So the example ships a path that can never resolve on any machine, including the authoring one `(verified, examples/dashboard-config.json, 2026-07-17)`. See [[routines]]' "Not portable" hazard.
