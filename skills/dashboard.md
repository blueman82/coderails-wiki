...
### SYSTEM VITALS gains two cost tiles (PR #185, 2026-07-15)
...
**Button's modal output rewritten to show the clean brief, not the trace (PR #183, 2026-07-15).** The button's `command` now also instructs the outer spawned `claude` agent to read the scratch file named on Rachel's `BRIEF FILE: <path>` sentinel line (added in the paired assistant-agent PR #27 to `tasks/inbox-brief.md`) and make its entire final message that file's verbatim contents. Necessary because the run-output modal renders the outer agent's `result` field — Rachel's own output is inner-layer text the modal never sees, so the sentinel had to be paired with an outer-agent instruction change, not shipped alone. See [[pr_181_183_dashboard-run-output-wrap-and-inbox-brief-clean-modal]].
