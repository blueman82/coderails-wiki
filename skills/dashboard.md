...
- [[crack_on_gate]] / [[sources/pr_175-176_crack-on-gate-and-inbox-brief-button]] — the sibling PR merged the same day (#175/#176): a new discipline hook plus the `inbox-brief` button, this config's first pure-dispatcher button (spawns `claude` only to shell out to `bin/rachel`, not to reason inline)
...
- [[enforcement-model]] — the headless-run exemption's ceiling framing (env-triggered, Stop-event-only, inside the agent's own trust domain; `SubagentStop` unaffected)
