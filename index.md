...
- [[pr_78_hooks-json-timeout-floor]] — PR #78 (merged 2026-06-29, ceabb75): closes the known gap from PR #76 — added `hooks/scripts/tests/hooks_json_timeout_floor.test.sh` asserting `min(declared hooks.json timeout) >= 5` (the `read -t 5` backstop floor). Fixes fractional-timeout silent-skip bug (bash `-lt` errors on floats; moved comparison to jq). Empty-result guard prevents vacuous-pass on broken jq filter. Suite 17/18 → 18/18
