...
**A third instance, against a root-owned install path rather than a working checkout (PR #274, 2026-07-22).** [[pr_274_tier_gate_observability_fixes|PR #274]]'s six tier-gate observability fixes were themselves caught by this exact gap during their own rollout: the daemon runs from an installed root copy at `/etc/coderails-tier-gate/tier-gate-runner.sh`, promoted there only by running `install.sh` — never from `origin/main` directly, so merging the PR did not by itself change what the live daemon executes. The first post-merge `install.sh` run promoted **stale** code, because the checkout it ran from was one commit behind `origin/main` (missing #274 itself) — the installed runner picked up an updated comment header but zero of the six functional fixes, while the installer still printed "INSTALL COMPLETE." Caught only by checking the live log's actual line format against what a genuine deploy should produce (the new per-tick timestamp prefix was absent), not by trusting the installer's success message; fixed by pulling the checkout current and re-installing. Same drift class as the two instances above — a property proven true of the source tree (tests green, PR merged) is not automatically true of the running process — but a new variant: the earlier two involve a `launchd` job pointed at a live working-checkout path, while this one involves a job pointed at an installed copy that a separate promotion step can itself run stale.
...
- [[pr_274_tier_gate_observability_fixes]] — PR #274 source record: six tier-gate daemon observability fixes, and a third MERGED ≠ DEPLOYED instance caught mid-rollout against the daemon's own installed root copy
