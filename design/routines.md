...
> **⚠️ Note (lint, 2026-07-17) — two open defects in the dated routines, neither fixed:**
> 1. **`{date}` is resolved in local time** by the runner while a `claude -p` run stamps UTC, so a correct run between 00:00–01:00 IST grades red against tomorrow's path. Affects all four dated routines.
> 2. **The example config's `loop-retro-promotion-weekly` `artifactPath` points at a checkout that does not exist** — it hard-codes `-Users-harrison-Documents-Github-coderails-.git`, but the checkout is `~/Github/coderails` (no `Documents/`), and the live config uses the `-Users-harrison-Github-...` key. So the shipped example carries a stale absolute path, on top of the machine-path rot described in [[loop-retro-promotion]]. Both are instances of the "Not portable" hazard below: these configs hard-code one machine's absolute paths `(verified, examples/dashboard-config.json vs. ~/.claude/coderails-dashboard.json, 2026-07-17)`.
