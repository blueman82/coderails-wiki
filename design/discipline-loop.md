...
> ⚠️ **CONTRADICTION, found 2026-07-17:** the "hardened in earlier PRs" parenthetical above is **not accurate** for `dc_extract_last_text` specifically. Verified directly against `origin/main`'s current `hooks/scripts/lib/discipline_common.sh`: `dc_extract_last_text` still runs `jq -R 'fromjson? // empty' | jq -s ... select(.type == "assistant") ...` with **no** `select(type == "object")` guard between the two stages — the exact vulnerable shape its sibling `als_extract_last_text` (`loop_state_common.sh`) had until [[pr_206_208_loop-state-common-docs-and-robustness|PR #208]] fixed it. `als_extract_last_text`'s own source comment explicitly cross-references `dc_extract_last_text` as sharing "the identical two-stage parse," and PR #208's body explicitly states `dc_extract_last_text` is "a separate, already-tracked, parked concern" left untouched — consistent with the memory handoff `project_jq_slurp_round2_handoff` ("2 remaining bare `jq -s` slurps"), inconsistent with this page's claim that it was "hardened in earlier PRs." Whichever earlier PR this sentence was describing either didn't cover `dc_extract_last_text` the way stated, or a regression re-introduced the gap — not re-derived here; flagging for a follow-up investigation rather than guessing which. Do not treat `dc_extract_last_text` as fixed until a PR actually lands the guard and this note is corrected.
