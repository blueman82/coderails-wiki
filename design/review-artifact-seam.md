...
**Comment-spoofing closed, `gh` pagination cap removed** ([[pr_7-10_task-evals-followups|PR #8]], 2026-07-06): `pr::has_coderails_review_for_head` now sources comment bodies from `pr::_trusted_comment_bodies`, which filters to the `gh`-authenticated login before any marker matching, and fetches via a paginated `gh api .../issues/<n>/comments --paginate` call rather than the old `gh pr view --json comments` GraphQL fetch (previously hard-capped at 100 comments). This PR's primary target was the eval-artifact reader (`pr::has_coderails_eval_for_head`, see [[task-evals-gate]]) but both readers share the fetch helper, so this seam inherits the same fix.

**Trust floor widened to a repo-permission check, org-repo limitation closed** ([[pr_11-14_gate-hardening-followups|PR #14]], `7c1dd19`, 2026-07-06, owner directive): PR #8's `author_association == "OWNER"` conjunct — which failed closed on org-owned repos, since the same authenticated user's own comments there carry `MEMBER`/`COLLABORATOR` instead of `OWNER` — is **removed entirely** (clean break, no surviving compat path). The trust rule is now: login matches the `gh`-authenticated identity (unchanged anti-spoof property) **AND** that identity holds write access or better (`ADMIN`/`MAINTAIN`/`WRITE`, via `gh repo view --json viewerPermission`) on the current repo — new `pr::_trusted_permission()` / `pr::_permission_is_write_or_better()` in `scripts/lib/git-common.sh`, shared by both PR-gate readers. Works identically on personal and org-owned repos now. A resolvable-but-insufficient permission is treated as "no trusted comment found" (not a fetch failure); an actual permission-lookup failure fails closed, same posture as an identity-fetch failure. `merge.sh`'s error messages for the two gates also gained a three-way split (identity-fetch vs. permission-fetch vs. comments-fetch failure) so a fetch-failed block now names which lookup failed. `INSTALLATION.md` documents the new trust rule for operators. See [[pr_11-14_gate-hardening-followups]] and [[task-evals-gate]] (the sibling seam sharing this same fetch helper).
