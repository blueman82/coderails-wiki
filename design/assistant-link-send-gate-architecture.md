...
- **Three of the four ASSISTANT.LINK panel slots remain unbuilt dependencies**: tasks due/overdue needs `assistant-agent/tasks/*.md` to actually be populated before a parse format can be derived (the directory was empty at spec time); email-last-checked needs a secretary-side sweep-timestamp state file that no work unit in this sub-project commits to building; routine-runs was deferred to sub-project 2. **Sub-project 2 has since shipped** ([[routines]], [[dashboard-runner]]) — the runner now writes `dashboard-runs/<routine>.md` vault run notes a routine-runs panel slot could read, but no panel-side work to actually render that slot is recorded as done here; this closes the *dependency* the spec was waiting on, not the panel slot itself. Only "sends + approvals log" (queue-backed) has a real data source as of this cluster.
