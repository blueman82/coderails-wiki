...
sources:
  ...
  - sources/pr_286_smoke_verify_array_guard.md
...
- ~~**`smoke_verify` (and checks 9/10) fail open on a non-array `.evals` — a scalar/string makes the `to_entries` index extraction jq-error to stderr with empty stdout, so the "no indices → return 0" early-return passed the merge gate re-executing nothing.**~~ **Closed by [[pr_286_smoke_verify_array_guard|PR #286]]** (`420004f`, 2026-07-23), the fix for the Critical [[pr_284_check10_hardening|PR #284]]'s review filed as a follow-up. A fail-closed type guard (`jq -e '(.evals | type) == "array"'`) after the tier-0 exemption in all THREE functions that iterate `.evals` — `smoke_verify`, `validate_smoke_execution` (check 10), `validate_smoke` (check 9). Guards on **type**, never on empty indices: a valid array whose only evals are `agent-run` legitimately has no scripted indices and must still be accepted. Non-obvious: check 7 (tier≥1 requires ≥1 P0) backstops the **scalar/string** shapes on the live `validate_structure` chain (`.evals[]?` finds no P0), but **NOT the object shape** — `.evals[]?` iterates an object's VALUES, so an object carrying a P0 passes check 7 and reaches the guard, which is the only thing that refuses it. The concrete recurrence of [[pr_261_freeze_before_build_gate|PR #261]]'s already-stated generalization above ("any check whose skip path keys on an empty read").
...
- [[pr_286_smoke_verify_array_guard]] — PR #286 (2026-07-23): fail-closed type guard on `.evals` in `smoke_verify` + checks 9/10, closing the non-array (scalar/string/object) fail-open [[pr_284_check10_hardening|#284]]'s review filed as a follow-up; load-bearing for the object shape check 7 doesn't backstop
